...
export default class ListTodo extends LightningElement {
    ...
    openAccordion(event) {
        let accordion = event.target.closest('.slds-accordion__section')
        let btn = event.target.closest('.todo-name')
        if (!accordion.classList.contains('slds-is-open')) {
            btn.childNodes[1].classList.add('btn-arrow-transform');
            accordion.classList.add('slds-is-open');
        } else {
            btn.childNodes[1].classList.remove('btn-arrow-transform')
